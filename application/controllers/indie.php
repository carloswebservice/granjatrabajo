...
class Indie extends CI_Controller {
    ...
    function datatable() {
        $this->datatables->select('id,ide_descripcion,ide_abreviatura,ide_estado')
                ->unset_column('id')
             //   ->add_column('logo', '<div style="height:30px;width:110px;"><img style="height:95%;" src="'.base_url()."public/img/empresas/".'$1" /></div>','logo')
                ...
                ->from("indicaciones_especiales")
                ->where('ide_estado',1);
        echo $this->datatables->generate();
    }

    public function json_getindie_id() {

        $id = $this->input->post('id');
        $obj = new Indiespecial();
    ...
    public function save(){
        if ($this->input->post()) {
            ...
            $obj->ide_descripcion = strtoupper($this->input->post("ide_descripcion"));
            $obj->ide_abreviatura = strtoupper($this->input->post("ide_abreviatura"));
            $obj->ide_estado = 1;
