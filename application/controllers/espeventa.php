...
class Espeventa extends CI_Controller {

    var $prefix = "espeventa";
    ...
    function datatable() {
        $this->datatables->select('id,edv_descripcion,edv_abreviatura,edv_estado')
                ->unset_column('id')
             //   ->add_column('logo', '<div style="height:30px;width:110px;"><img style="height:95%;" src="'.base_url()."public/img/empresas/".'$1" /></div>','logo')
                ...
                ->from("especificacion_de_venta")
                ->where('edv_estado',1);
        echo $this->datatables->generate();
    }

    public function json_getespeventa_id() {

        $id = $this->input->post('id');
        $obj = new Especificacionventa();
    ...
    public function save(){
        if ($this->input->post()) {
            ...
            $obj->edv_descripcion = strtoupper($this->input->post("edv_descripcion"));
            $obj->edv_abreviatura = strtoupper($this->input->post("edv_abreviatura"));
            $obj->edv_estado = 1;
       ...
       public function delete(){
        ...
        if($obj->save()){
                $newobl = new Log();
                $newobl->log_usuario = $_SESSION["id_usuario"];
                $newobl->log_tabla = "especificacion_de_venta";
                $newobl->log_fecha = date('Y-m-d H:m:s');
