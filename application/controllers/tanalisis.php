...
class Tanalisis extends CI_Controller {

    var $prefix = "tanalisis";
    ...
    function datatable() {
        $this->datatables->select('id,tpa_descripcion,tpa_abreviatura,tpa_estado')
                ->unset_column('id')
    ...
    public function json_gettanalisis_id() {

        $id = $this->input->post('id');
        $obj = new Tipoanalisis();
    ...
    public function save(){
        if ($this->input->post()) {
            ...
            $obj->tpa_descripcion = strtoupper($this->input->post("tpa_descripcion"));
            $obj->tpa_abreviatura = strtoupper($this->input->post("tpa_abreviatura"));
            $obj->tpa_estado = 1;
       ...
       public function delete(){
        ...
        if($obj->save()){
                $newobl = new Log();
                $newobl->log_usuario = $_SESSION["id_usuario"];
                $newobl->log_tabla = "tipo_analisis";
                $newobl->log_fecha = date('Y-m-d H:m:s');
